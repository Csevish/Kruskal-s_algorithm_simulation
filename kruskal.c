...
void display_hello()
{
	...
	writeDistance("Press ENTER key to Start the Dijkstra's Algorithm.", 300,200);
}
...
void fixAdjMatrix()
{
	for(int i = 0; i < maxNodes; i++)
	{
		...
		{
			if (j == i){adjMatrix[i][j] = 0; continue;}
			adjMatrix[i][j] = 99999;
		}
	}
}
